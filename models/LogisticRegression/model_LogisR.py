...
def make_model(X_train, X_test, Y_train, Y_test, A_train, A_test):
    ...
    logreg.fit(layer_train, Y_train)
    print('LogisticRegression fit complete')

    predicts = logreg.predict(layer_test)
    predict_prob = logreg.predict_proba(layer_test)[:,1]
    logloss = log_loss(Y_test, predict_prob)
    train_acc = logreg.score(layer_train, Y_train)
    test_acc = logreg.score(layer_test, Y_test)
